    ...
    <div id="main-wrapper">
      ...
      <?php include('includes/sidebar.php');?>
		...
		<select class="select2 form-control custom-select chosen-select" name="order" id="order" required onchange="this.form.submit();">
		  ...
		  $order = mysqli_query($zconn,"select distinct order_no from order_entry_master where (`order_no`,`style_no`) NOT IN(select order_no,style_no from accessories_planning_list)  and (`order_no`,`style_no`) IN (select order_no,style_no from process_planning_flow)");
		...
		<tbody>
					...
					<?php 
							...
							if (isset($_REQUEST['order'])) {
								...
								$order_det=mysqli_query($zconn, "SELECT * FROM order_entry_master WHERE order_no='$order_no'");
								$order_data = mysqli_fetch_array($order_det, MYSQLI_ASSOC);
...
while ($order_data = mysqli_fetch_array($order_det, MYSQLI_ASSOC)) {
    ...
    if ($cutting_det_result) {
       ...
       // echo "Size: $size_id,  Order Qty : $size_qty,Cutting Qty : $cutting_qty_val";
        if ($cutting_qty_val !== '') {
           // echo ",  Cutting Qty : $cutting_qty_val <br>";
        } else {
            echo ", No Cutting Quantity found <br>";
        }
    } else {
        // Handle query errors
        echo "Error in fetching cutting quantity details";
    }
    // You can access other columns in $order_data similarly
}
		...
		<td>
			<select class="select2 form-control custom-select uom" name="uom[]"  style="width:80px;" > 
				<option>Select</option>
    ...
    while ($order_data = mysqli_fetch_array($order_det, MYSQLI_ASSOC)) {
	   ...
	   echo "$cutting_qty_val<br><input type='checkbox' name='selected_items_cutting_qty[]' value='$cutting_qty_val' onchange='calculateTotal()'>";
        ...
        // Adding a checkbox column for each cell
      //  echo '<td>'.$size_id.'<br><input type="checkbox" name="selected_items_size[]" value="'.$size_id.'"></td>';
       // echo '<td><input type="checkbox" name="selected_items_qty[]" value="'.$size_qty.'"></td>';
       // echo '<td><input type="checkbox" name="selected_items_cutting_qty[]" value="'.$cutting_qty_val.'"></td>';
       // echo '<td><input type="checkbox" name="selected_items_color[]" value="'.$color.'"></td>';
...
<!--td>
    <input type="text" class="totalQty" value="<?php echo $excess_cal; ?>">
</td-->
<!-- ... -->
...
function calculateTotal() {
    ...
    rows.forEach(function(row) {
        var checkboxesQty = row.querySelectorAll("input[name='selected_items_qty[]']");
        var checkboxesCuttingQty = row.querySelectorAll("input[name='selected_items_cutting_qty[]']");
        var total = 0;

        for (var i = 0; i < checkboxesQty.length; i++) {
            if (checkboxesQty[i].checked) {
                var orderQty = parseInt(checkboxesQty[i].value);
                var cuttingQty = checkboxesCuttingQty[i].checked ? parseInt(checkboxesCuttingQty[i].value) : 0;
                total += orderQty + cuttingQty;
            }
        }

        var totalQtyInput = row.querySelector('.totalQty');
        if (totalQtyInput && total > 0) { // Only update the value if total > 0
	...
	<td>
    <input type="text" name="order_qty[]" class="form-control order_qty totalQty" id="" value="<?php echo $excess_cal;?>" onkeyup="cal_total();" onblur="cal_total();">
</td>
<td>
...
<input type="hidden" class="form-control" id="order_tot" value="<?php echo $sel_fabric['order_qty']; ?>" readonly placeholder="" ></td>
				...
				<td>
				<input type="text" class="form-control" id="grand_tot" name="grand_tot"  placeholder="" value="<?php echo $order;?>" style="border: 1px solid #000;">
				</td>
				<td>
