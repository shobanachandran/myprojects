...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save'])) {
    ...
    if (!empty($insertData)) {
        foreach ($insertData as $data) {
            ...
            // Insert data into the 'cutting_balance' table
            $insertCuttingBalanceQuery = "INSERT INTO cutting_balance (style_no, order_no, dc_no, party_dc_no, to_contractor, `from`, dc_out_date, size_id, color, qty_val, `session_entry`) VALUES ('$style_no', '$order_no', '$dc_no','$party_dc_no', '$to_contractor', '$from', '$dc_date', '$size_id', '$color', $qty_val, '$uname')";

            $resultCuttingBalance = mysqli_query($zconn, $insertCuttingBalanceQuery);
            ...
            if (!$resultCuttingBalance || !$resultDc) {
