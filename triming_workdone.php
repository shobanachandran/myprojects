    ...
    <table class="table table-striped table-bordered text-center">
        <thead>
            <tr>
                ...
                $sectBrnQry = "SELECT DISTINCT size_id FROM triming_workassign WHERE order_no='" . $_REQUEST['order'] . "'";
        ...
        <tbody>
            ...
            $sectBrnQry = "SELECT DISTINCT color FROM triming_workassign WHERE order_no='" . $_REQUEST['order'] . "'";
            $secBrnResource = mysqli_query($zconn, $sectBrnQry);
            ...
            $sectBrnQry = "SELECT * FROM triming_workassign WHERE order_no='" . $_REQUEST['order'] . "'";
            $secBrnResource = mysqli_query($zconn, $sectBrnQry);
...
<div class="table-responsive">
    ...
    <table class="table table-striped table-bordered text-center">
        ...
        <tbody>
            ...
            $sectBrnQry = "SELECT order_no, color, size_id, qty_val FROM triming_workassign WHERE order_no='" . $_REQUEST['order'] . "'";
            $secBrnResource = mysqli_query($zconn, $sectBrnQry);
            while ($coldata = mysqli_fetch_array($secBrnResource, MYSQLI_ASSOC)) {
            ...
            $sectBrnQry = "SELECT order_no, color, size_id, SUM(qty_val) AS total_qty FROM triming_workdone WHERE order_no='" . $_REQUEST['order'] . "' GROUP BY order_no, color, size_id";
            $secBrnResource = mysqli_query($zconn, $sectBrnQry);
            while ($coldata = mysqli_fetch_array($secBrnResource, MYSQLI_ASSOC)) {
   ...
   // var_dump($_POST); // Add this line to check the received POST data
    ...
    if (!empty($insertData)) {
        foreach ($insertData as $data) {
            ...
            // Insert data into the 'triming_workdone' table
            $insertCheckingWorkdoneQuery = "INSERT INTO triming_workdone (style_no, order_no, dc_no, party_dc_no, to_contractor, `from`, dc_out_date, size_id, color, qty_val, `session_entry`) VALUES ('$style_no', '$order_no', '$dc_no','$party_dc_no', '$to_contractor', '$from', '$dc_date', '$size_id', '$color', $qty_val, '$uname')";
