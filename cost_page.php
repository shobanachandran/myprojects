...
?>
        <tr>
            ...
            <td style="border: 1px solid; width: 23%; padding: 3px; margin: 0; text-align: center;">
                <?php
                echo '₹&nbsp;', $res_cost_det['rate_per_kg'];
                $grant_total_knitting1 += $res_cost_det['rate_per_kg'];
                ?>
            </td>
            <td style="border: 1px solid; width: 23%; padding: 5px; margin: 0; text-align: center;">
                <?php
                echo '₹&nbsp;', $res_cost_det['rate_per_pc'];
                ?>
            </td>
        </tr>
<?php
...
$count = mysqli_fetch_array(mysqli_query($zconn, "SELECT * FROM costing_entry_details WHERE costing_id='" . $costing_no . "' AND fabric_name!=''"), MYSQLI_ASSOC);

while ($res_cost_det = mysqli_fetch_array($cost_det, MYSQLI_ASSOC)) {
    $fabric_name = $res_cost_det['fabric_name'];
	
	    $cost_query = "SELECT COUNT(fabric_name) as count, SUM(total) as total FROM dyeing_costing WHERE costing_no='" . $costing_no . "' AND fabric_name!=''";
    $cost_result = mysqli_query($zconn, $cost_query);
    $cost = mysqli_fetch_array($cost_result, MYSQLI_ASSOC);

    $total = $cost['total'] / $cost['count'];
?>
    <tr>
    <td style="border: 1px solid; width: 23%; padding: 3px; margin: 0; text-align: center;"><?php echo  $res_cost_det['fabric_type']; $total_rate += $res_cost_det['rate']; ?></td>

        <td style="border: 1px solid; width: 23%; padding: 3px; margin: 0; text-align: center;"><?php echo '₹&nbsp;', $res_cost_det['rate']; $total_rate += $res_cost_det['rate']; ?></td>
        <td style="border: 1px solid; width: 23%; padding: 5px; margin: 0; text-align: center;"><?php echo '₹&nbsp;', $res_cost_det['rate_pcs']; $grant_total += $total; $total_rate1 += number_format($total, 2); ?></td>
    </tr>
<?php
    $sn++;
}
