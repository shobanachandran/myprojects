...
if (isset($_POST['save'])) {
    ...
    // Delete existing data related to the costing entry if needed
  ...
  //  $insertListQuery = mysqli_query($zconn, "INSERT INTO other_costing_list (costing_no, order_no, style_no, total, created_by, created_date, buyer)
    ...
    for ($i = 0; $i < $fabric_name_count; $i++) {
        ...
        $selCostQuery = mysqli_query($zconn, "SELECT SUM(consumption_percent) AS consumption_percent, SUM(pcs_weight) AS pcs_weight 
        ...
        $row = mysqli_fetch_assoc($sel_check);
        $current_id = $row['id'];

        // Update the existing record in other_costing based on the unique ID
							...
							$sel_costing = mysqli_query($zconn,"select * from costing_entry_master where id IN(select costing_no from other_costing_list) ");						
										...
										<?php } ?>
											...
											<div class="form-group row">
												...
												?>
												<label for="lname" class="col-sm-3 text-right control-label col-form-label">Intent No</label>
												<div class="col-sm-6">
										...
										<div class="card" style="width:50%; float:left; right: 50px;">
											...
											<div class="form-group row">
												...
												<div class="col-sm-6">
											<?php if($_REQUEST['costing_no']!=''){
													$date_arr =explode("-",$sel_entry1['created_date']);
													$date_created = $date_arr['2']."-".$date_arr['1']."-".$date_arr['0'];
													?>
												<input type="text" class="form-control"  value="<?php echo $costing_date; ?>" readonly id="order_date">
											<?php } else{
													?>
								...
								<?php 
										...
										$sel_cost = mysqli_query($zconn,"select distinct(fabric_name)as fabric_name from costing_entry_details where costing_id='".$cos_no."'");
										while($res_costing = mysqli_fetch_array($sel_cost,MYSQLI_ASSOC)){
											$sel_cost00 = mysqli_fetch_array(mysqli_query($zconn,"select sum(pcs_weight) as pcs_weight  from costing_entry_details WHERE fabric_name ='".$res_costing['fabric_name']."' and `costing_id` = '".$cos_no."'"));
										?>
									<tr id="example2">
										<td>
										...
										<td>
											<input type="text" class="form-control" id="fname" readonly value="<?php $color=mysqli_query($zconn,"select * from costing_entry_details where fabric_name ='".$res_costing['fabric_name']."' and `costing_id` =  '".$_REQUEST['costing_no']."'");while($clrdata=mysqli_fetch_array($color,MYSQLI_ASSOC)){$clr = $clrdata['yarn_colour'].",";echo $clr;}?>">
										</td>
										<td>
											<input type="text" class="form-control" id="fname"  value="<?php echo number_format($sel_cost00['pcs_weight'],2);?>">
										</td>
									</tr>
								<?php } ?>
											</tbody>
...
<td style="width: 15%" >&nbsp;</td><td style="width: 15%">&nbsp;</td><td style="width: 15%">&nbsp;</td><td style="width: 11%">&nbsp;</td><td style="width: 11%">&nbsp;</td>
												...
												<!--td><h6 class="page-title">Total Weight:</h6></td>
												<td>
													<input type="text" class="form-control" id="total_wgt" value="<?php echo $totalwgt;?>" readonly placeholder="" style="border: 1px solid #000;">
												</td>-->
