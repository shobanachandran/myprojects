...
?>
<?php
$recipt_data=mysqli_fetch_object(mysqli_query("SELECT * FROM `credit_note` ORDER BY `credit_note`.`i` DESC"));
$credit_no=$recipt_data->receipt_no+1;
$credit_date=date("d-m-Y");

if(isset($_REQUEST['save_credit_note'])){
	...
	$credit_date=date("Y-m-d",strtotime($_REQUEST['receipt_date']));
		...
		for($sno=0;$sno<$update_rows;$sno++){
			$get_my_datas=mysqli_fetch_object(mysqli_query("SELECT * FROM `credit_note_session` WHERE `i` = '".$_REQUEST['sno'][$sno]."'"));
			...
			mysqli_query("INSERT INTO `credit_note` (`i`, `receipt_no`, `receipt_date`, `sessid`, `customer_code`, `main_amount`, `invoice_no`, `invoice_date`, `amount`) VALUES (NULL, '$credit_no', '$credit_date', '$sessid', '$customer_code', '$main_amount', '$bill_no[0]', '$bill_date', '$bill_amount')");
			
				//$inv_data=mysqli_fetch_object(mysqli_query("SELECT * FROM `invoice` WHERE `invoice_no` = $bill_no[0]"));
				//$inv_paid=$inv_data->amount_paid + $bill_amount1;
				//mysqli_query("UPDATE `invoice` SET `amount_paid` = '$inv_paid' WHERE `invoice`.`invoice_no` = $bill_no[0]");
	
		}
		
		if($bill_amount1 > '0'){
			mysqli_query("INSERT INTO `credit_note` (`i`, `receipt_no`, `receipt_date`, `sessid`, `customer_code`, `main_amount`, `invoice_no`, `invoice_date`, `amount`) VALUES (NULL, '$credit_no', '$credit_date', '$sessid', '$customer_code', '$main_amount', '".$bill_no1[0]."', '$bill_date1', '$bill_amount1')");

			//$inv_data=mysqli_fetch_object(mysqli_query("SELECT * FROM `invoice` WHERE `invoice_no` = '".$bill_no1[0]."'"));
			//$inv_paid=$inv_data->amount_paid + $bill_amount1;
			//mysqli_query("UPDATE `invoice` SET `amount_paid` = '$inv_paid' WHERE `invoice`.`invoice_no` = '".$bill_no1[0]."'");
			
		}
		
		
	echo '<script>alert("Debit Added Successfully");</script>';
	echo '<meta http-equiv="refresh" content="0;URL=credit_note.php?sessid='.$random.'">';	
}
if(isset($_REQUEST['delete'])){
	$delete=$_REQUEST['delete'];
	$sessid=$_REQUEST['sessid'];
	mysqli_query("DELETE FROM `credit_note_session` WHERE `credit_note_session`.`i` = $delete")or die(mysqli_error());
	echo '<script>alert("Deleted Successfully");</script>';
	echo'<meta http-equiv="refresh" content="0;URL=credit_note.php?sessid='.$sessid.'">';	
}
if(isset($_REQUEST['add_row'])){
	...
	if(isset($_REQUEST['sno'])){
		...
		for($sno=0;$sno<$update_rows;$sno++){
			...
			mysqli_query("UPDATE `credit_note_session` SET `amount` = '$bill_amount1' WHERE `credit_note_session`.`i` = '".$_REQUEST['sno'][$sno]."'")or die(mysqli_error());	
		}
		
	}	
	$check=mysqli_num_rows(mysqli_query("SELECT * FROM `credit_note_session` WHERE `sessid` = '$sessid' AND `invoice_no` = '$bill_no'"));
	if($check == '0'){
		if($bill_no != ''){
	mysqli_query("INSERT INTO `credit_note_session` (`i`, `sessid`,  `customer_code`, `main_amount`, `invoice_no`, `invoice_date`,`amount`) VALUES (NULL, '$sessid',  '$did','$main_amount', '$bill_no', '$bill_date', '$bill_amount')")or die(mysqli_error());
		}
		else{
		...
		echo '<script>alert("Allredy Exists please add me another recipt no");</script>';
	}
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?php echo SITE_TITLE;?> - Credit Note</title>
    <!-- Custom CSS -->
    <!-- Custom CSS -->
                        ...
                        <div class="ml-auto text-right">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page"><a href="#">Credit Note</a></li>
                                </ol>
                            </nav>
                        </div>
                    </div>
...
<!--	<div class="box-header well" data-original-title>
  ...
  <tr>
    <td width="20%" height="35" align="right"><strong>Credit Receipt no&nbsp;:&nbsp;</strong></td>
  ...
  <tr>
    ...
    <td><select name="did" class="span7" data-rel="chosen" onchange="this.form.submit()">
              <option value="">----Select----</option>
            	<?php 
               $select_name = mysqli_query("SELECT * FROM  `buyer_master`");
               while($select_name1 = mysqli_fetch_object($select_name))
               { ?>	
               <option value="<?php echo $select_name1->buyer_name;?>"<?php //if($select_name1->customercode == $_REQUEST['did']){?> selected="selected"<?php //$direct='false';$select_name1distributorcode=$select_name1->customercode;$agent=$select_name1->typeofcustomer;}?>><?php //echo $select_name1->customername;?></option>
        	<?php }  ?> 
          </select></td>
  ...
  <tr>
    <td colspan="6"><table class="table table-striped table-bordered" width="100%" border="1" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td align="center"><strong>Ref / Date</strong>
		</td>
        <td align="center"><strong>Amount</strong></td>
        <td align="center"><input type="submit" name="add_row" class="btn btn-inverse" value="Add me" /></td>
      </tr>
      <?php
	  $sql=mysqli_query("SELECT * FROM `debit_note_session` where `sessid` = '".$_REQUEST['sessid']."'");
	  while($res=mysqli_fetch_array($sql)){
	  ?>
      <tr><input type="hidden" name="sno[]" value="<?php echo $res['i'];?>" />
        <td align="left"><?php echo $res['invoice_no'];?></td>
        <td align="center"><input type="text" name="bill_date1[]" readonly id="costing_date" value="<?php //echo $res['invoice_date'];?>" class="form-control span8" /></td>
      ...
      <tr>
        ...
        <option value="">----Select----</option>
        <?php
        $sql=mysqli_query("SELECT * FROM `invoice` WHERE `customer_code` LIKE '".$_REQUEST['did']."' ORDER BY `customer_code` ASC");
 ...
 function calculation_sc(){
	  ...
	  <?php $sql=mysqli_query("SELECT * FROM `credit_note_session` where `sessid` = '".$_REQUEST['sessid']."'");
	  while($res=mysqli_fetch_array($sql)){?>
	  var bill_session_<?php echo $res['i'];?>=parseInt(document.getElementById('bill_amount1_<?php echo $res['i'];?>').value);
	  <?php }?>

	  var total_amount = <?php $sql=mysqli_query("SELECT * FROM `credit_note_session` where `sessid` = '".$_REQUEST['sessid']."'");
 ...
 function check_me_balance(){
	...
	if(balancee == '0'){
		var r = confirm("To confirm this amount credit note..!");
		if (r == true) {
			return true;
		} else {
			return false;
