...
<?php
    ...
    if (isset($_POST['save'])) {
        ...
        if (isset($_POST['style_no']) && isset($_POST['desc']) && isset($_POST['accessories_name']) &&  isset($_POST['pcs_wgt']) && isset($_POST['price'])) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            for ($i = 0; $i < count($_POST['style_no']); $i++) {
                ...
                $accessories_name = mysqli_real_escape_string($zconn, $_POST['accessories_name'][$i]);
...
`to_process`, `cgst`,`sgst`,`igst`, `style_no`, `desc`, `accessories_name`,  `pcs_wgt`, `price`, `status`, `pay_status`)
  VALUES ('$po_no',  NOW(), '$received_date','$to_address', '$comments', '$delivery_to', '$to_process','$cgst','$sgst','$igst','$style_no', 
  '$desc', '$accessories_name', '$pcs_wgt', '$price', 'pending','pending')";
...
<legend>Store Details</legend>
    ...
    <th width="5%">
                    ...
                    <button type="button" class="btn btn-danger btn-sm ml-2" onclick="addRow2()"><i class="fas fa-plus-circle fa-lg"></i></button>
                </th>			</tr>
        </thead>
