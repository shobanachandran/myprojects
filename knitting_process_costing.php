								...
								<div class="card" style="width:50%; float:left; left: 50px; " >
									<div class="form-group row">
										...
										<div class="col-sm-6">
											...
											name="costing_no" id="costing_no" onchange="this.form.submit()">
												<option>Select</option>
												<?php if($_REQUEST['id']==''){
						$sel_costing = mysqli_query($zconn,"SELECT *
						FROM costing_entry_master
						WHERE id NOT IN (
							SELECT costing_no
							FROM knit_costing_list
						) 
						AND cost_type = 'yarn'
						");
												} else {
							$sel_costing = mysqli_query($zconn,"select * from costing_entry_master where id IN(select costing_no from knit_costing_list) AND cost_type = 'yarn' ");						
												}
        ...
        th {
            ...
            text-align: center;
							...
							<tr>
								...
								<th style="width:10%;">Fabric Component</th>
								<th style="width:10%;">Total Weight</th>
								<th style="width:10%;">Unit</th>
								<th style="width:10%;">Rate/KG	</th>
								<th style="width:10%;">Rate/Pcs</th>
				...
				<td><input type="text" name="uom[]" id="comp_group0" value="<?php echo $res_cost_det['uom_id'];?>" class="form-control" readonly></td>
										<td>
...
while ($res_cost_det = mysqli_fetch_array($cost_det, MYSQLI_ASSOC)) {
	...
	$pcs_weight_values = implode(", ", $cons_value); 
						...
						<input type="text" name="fab_name[]" value="<?php echo $res_cost_det['fabric_name'];?>" class="form-control" readonly >
							...
							<?php 
								...
								while($res_fab=mysqli_fetch_array($fab_sql,MYSQLI_ASSOC)){
									...
									<td>
									<input type="text" name="fab_type[]" readonly value="<?php echo $res_cost_det['comp_id'];;?>" style="border:none;">
										<!--<select class="select2 form-control custom-select" name="fab_type[]">
										<option>Select</option>
										<?php $selcomp = mysqli_query($zconn,"select * from components where status='0'");
										while($res_comp = mysqli_fetch_array($selcomp,MYSQLI_ASSOC)){
    ...
    //echo $value; // Display each value of $cons_values
?>

										<input type="text" class="form-control single"  id="<?php echo 'yarn_wgt_'.$rec; ?>" name="yarn_weight[]" autocomplete="off" value="<?php echo $res_cost_det['pcs_weight'];?>"  readonly  >
