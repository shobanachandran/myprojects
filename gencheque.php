...
<body>
    <div id="main-wrapper">
        ...
        <div class="page-wrapper" style="min-height: 100%; height: auto;">
			...
			<div class="container-fluid">               
                ...
                <table class="table table-striped table-bordered" width="75%" border="0" align="center" cellpadding="0" cellspacing="0">
				<tr>
				<td align="right">Cheque By</td>
				<td align="center">&nbsp;</td>
				...
				<option value="fiat">JOBWORK</option>
                        <?php
                     /*   $dccc=mysql_query("SELECT DISTINCT(supplier_id) as id FROM  `bill_pass_purchase`");
						while($dccs=mysql_fetch_object($dccc)){
						$supply_name=mysql_fetch_object(mysql_query("SELECT *  FROM `master_supplier` WHERE `customercode` = '$dccs->id'"));
						?>
                <option value="<?php echo $dccs->id;?>" <? if(in_array($dccs->id,$_REQUEST['company'],true)){?> selected="selected"<? }?>><? echo $supply_name->customername;?></option>
                        <?
                        }
						*/?>
                        </select>
				...
				<tr>
				<td align="right">Choose Company</td>
				<td align="center">&nbsp;</td>
				<td><select name="company[]" data-rel="chosen" class="span10" onchange="this.form.submit()">
				<option value="">----Select----</option>
                        <?php
    ...
    <!--<tr>
      ...
      <td width="48%"><select name="dcno[]" multiple data-rel="chosen" class="span10" onchange="this.form.submit()">
                        <?
                        $dccc=mysql_query("SELECT DISTINCT(prime_id) as id FROM  `bill_pass_purchase` where `supplier_id` = '".$_REQUEST['company'][0]."' and `mode` =''");
						while($dccs=mysql_fetch_object($dccc)){
						?>
                <option value="<?php echo $dccs->id;?>" <? if(in_array($dccs->id,$_REQUEST['dcno'],true)){?> selected="selected"<? }?>><? echo $dccs->id;?></option>
                        <?
                        }
						?>
                        </select></td>
    ...
    <tr>
	  ...
	  foreach($_REQUEST['dcno'] as $dcno){
		  $sql=mysql_fetch_object(mysql_query("SELECT * FROM  `bill_pass_purchase` WHERE  `prime_id` = '$dcno'"));
	  ...
	  foreach($_REQUEST['company'] as $supplier_ids){//echo $supplier_ids.'<br>';
		$company_namess=mysql_fetch_object(mysql_query("select * from `master_supplier` where `customercode` = '$supplier_ids'"));
	...
	$datas=mysql_query("SELECT * FROM  `bill_pass_purchase` WHERE  `supplier_id` ='$dcno' and `mode` = ''");
