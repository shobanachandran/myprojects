...
if(isset($_POST['save'])){
	...
	for($i=0;$i<$trows;$i++){
		$insert_fab_costing = mysqli_query($zconn,"insert into dyeing_costing(costing_no,color_name,weight,rate,rate_pcs,total,created_by,created_date,fabric_name) values('".$costing_no."','".$_POST['colour'][$i]."','".$_POST['weight'][$i]."','".$_POST['rate'][$i]."','".$_POST['rate_pcs'][$i]."','".$_POST['total'][$i]."','".$_SESSION['userid']."',now(),'".$_POST['fabric_name'][$i]."')");
	}
                        ...
                        <div class="ml-auto text-right">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page"><a href="#">Dyeing Process Costing Info</a></li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                        ...
                        <div class="card">
								<div class="card-body" style="width:100%">
									<div class="card-body" style="width:100%">
										...
										<?php $sel_costing1 = mysqli_query($zconn,"select * from costing_entry_master");
										...
										<div class="card" style="width:50%; float:left; right: 50px;">
											...
											<div class="form-group row">
												...
												<div class="col-sm-6">
													...
													<?php 
													if($_REQUEST['costing_no']==''){ ?>
														<input type="date" class="form-control" name="created_date" readonly id="cono1" value="<?php echo $sel_buyer1['costing_date'];?>">
														<?php } else{ ?>

													<input type="date" class="form-control" name="created_date" readonly id="cono1" value="<?php echo $sel_costing['costing_date'];?>">
												<?php } ?>
										...
										<table class="table">
											...
											if ($_REQUEST['costing_no']=='') {
												...
												$sel_cost = mysqli_fetch_array(mysqli_query($zconn,"select sum(consumption_percent) as consumption_percent,sum(pcs_weight) as pcs_weight  from costing_entry_details WHERE fabric_name ='".$res_det1['fabric_name']."' and `costing_id` =  '".$id."'"));
											 ?>
												<tr id="example2">
												<!-- <td>
									...
									<table id="example" class="table table-striped table-bordered">
										...
										<tbody>
											...
											<tr id="delete_0">
												<td>
													<select class="select2 form-control custom-select" data-no="0" name="fabric_name[]" id="fabric_name" onchange="fabric(this);">
														<option value="<?php echo $res_det11['fabric_name'];?>"><?php echo $res_det11['fabric_name'];?></option>
														<option>--Select--</option>
														<?php $sel_cost_details1 = mysqli_query($zconn,"select distinct(fabric_name)as fabric_name from costing_entry_details WHERE  `costing_id` =  '".$id."'");
												while($res_det1 = mysqli_fetch_array($sel_cost_details1,MYSQLI_ASSOC)) { 
												?>
												...
												<td>
													<select class="select2 form-control custom-select" name="colour[]">

														<option value="<?php echo $res_det11['color_name'];?>"><?php echo $res_det11['color_name'];?></option>
														<option>Select</option>
										...
										<tbody>
											...
											<?php } else{ 
												$sel_cost_details1 = mysqli_query($zconn,"select distinct(fabric_name)as fabric_name from costing_entry_details WHERE  `costing_id` =  '".$_REQUEST['costing_no']."'");
												while($res_det1 = mysqli_fetch_array($sel_cost_details1,MYSQLI_ASSOC)) { 
												$sel_cost = mysqli_fetch_array(mysqli_query($zconn,"select sum(consumption_percent) as consumption_percent,sum(pcs_weight) as pcs_weight  from costing_entry_details WHERE fabric_name ='".$res_det1['fabric_name']."' and `costing_id` =  '".$_REQUEST['costing_no']."'"));
											 ?>
												<tr id="example2">
												<!-- <td>
														<input type="text" class="form-control" id="fname" readonly value="<?php echo $res_det1['yarn_name'];?>">
													...
													<td>
														<input type="text" class="form-control" id="fname" readonly value="<?php 
														$totclr=mysqli_query($zconn,"select * from costing_entry_details WHERE fabric_name ='".$res_det1['fabric_name']."' and `costing_id` =  '".$_REQUEST['costing_no']."'");
														while($clr=mysqli_fetch_array($totclr)){
														echo $clr['yarn_colour'].",";	
														}
														?>">
												...
												<!--	<td>
													...
													<td>
														<input type="text" class="form-control" id="fname"  value="<?php echo number_format($sel_cost['pcs_weight'],2);?>">
													</td>
												</tr>
											<?php } ?>
										</table>
...
function cal_amt(id){
		...
		if(rat!=''){
			...
			single=ctrws/cons;
			var rowcount=$("#rowtot").val();
			$('#rate_pcs'+id).val(single);
		} 
		$('#total'+id).val(ctrws);
		$('.totl').each(function(){
			sm += parseFloat($(this).val());
...
$(document).ready(function(){
	...
	//	$(this).attr("disabled", "disabled");
        ...
        var row = '<tr>' +
            '<td><select class="select2 form-control custom-select" data-no="'+id+'" name="fabric_name[]" id="fabric_name'+id+'" onchange="fabric(this);"><option>--Select--</option><?php if($_REQUEST['costing_no']==''){ $sel_cost_details1 = mysqli_query($zconn,"select distinct(fabric_name)as fabric_name from costing_entry_details WHERE  `costing_id` =  '".$id."'");while($res_det1 = mysqli_fetch_array($sel_cost_details1,MYSQLI_ASSOC)) {?><option value="<?php echo $res_det1['fabric_name'];?>"><?php echo $res_det1['fabric_name'];?></option><?php } }else{ $sel_cost_details1 = mysqli_query($zconn,"select distinct(fabric_name)as fabric_name from costing_entry_details WHERE  `costing_id` =  '".$_REQUEST['costing_no']."'");while($res_det1 = mysqli_fetch_array($sel_cost_details1,MYSQLI_ASSOC)) {?><option value="<?php echo $res_det1['fabric_name'];?>"><?php echo $res_det1['fabric_name'];?></option><?php } }?> </select></td>'+'<td><select class="select2 form-control custom-select" name="colour[]"><option> Select</option>'+color_list+'</select></td>' + '<td><input type="text" class="form-control wei" readonly id="weight_'+id+'" placeholder="Weight" name="weight[]" autocomplete="off" onkeyup="cal_amt('+id+');" onkeyup="cal_amt('+id+');"></td>' +
