...
if (isset($_POST['selectedOption']) && $_POST['selectedOption'] === 'yarn') {
    // Fetch supplier names from the database for 'fabric'
    $sql = "SELECT supplier_id, supplier_name FROM suppliers WHERE supplier_type_id = (SELECT supplier_type_id FROM supplier_types WHERE supplier_type = 'yarn')";
