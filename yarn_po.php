...
$po_sql = mysqli_fetch_array(mysqli_query($zconn,"select max(id) as id from yarns_po_master"));
	...
	$total_bag_wgt = $_REQUEST['bag'] * $_REQUEST['total_bag'];

	$sql1 = mysqli_query($zconn,"INSERT INTO `yarns_po_master` (`po_no`,`buyer`,
	`deliver_to`,`cgst`,`sgst`,`igst`,`grant_total`,`status`,`date`,`kg_bag`,`total_bag`,`comments`,`receive_date`,`total_bag_wgt`)	
	VALUES('".$po_no."','".$_REQUEST['to_address']."','".$_REQUEST['delivery']."','".$_REQUEST['cgst']."','".$_REQUEST['sgst']."', '".$_REQUEST['igst']."','".$_REQUEST['grant_total']."','SEND','".$_REQUEST['po_date']."','".$_REQUEST['bag']."','".$_REQUEST['total_bag']."','".$_REQUEST['comments']."','".$_REQUEST['receive_date']."','".$total_bag_wgt."')")or die(mysqli_error());
				...
				for($i=0;$i<count($_REQUEST['now']);$i++){
					...
					//	`yarn_entry_details`(`id`, `yarn_id`, `costing_no`, `yarn_name`, `yarn_count`, `yarn_type`, `yarn_colour`, `yarn_content`, `fabric_name`, `comp_group`, `comp_id`, `consumption_value`, `consumption_percent`, `pcs_weight`, `uom_id`, `yarn_rate`, `yarn_total`, `rate_perkg`, `rate_total`

						$sql1 = mysqli_query($zconn,"INSERT INTO `yarns_po_details` (`po_id`,`styleno`,
						 `order_no`, `weight`, `rate`,`grant_total`,`date`,`cutting_qty`,`balance_qty`,`counts`,`yarn_name`,`yarn_colour`, `yarn_content`, `pcs_weight`,`consumption`)VALUES('$last_id', '$style_no', '$order_no', '$now_qty','$pcs_rate','".$_REQUEST['grant_total']."',Now(),'".$_REQUEST['cutting_qty'][$i]."','".$_REQUEST['balance'][$i]."','".addslashes($_REQUEST['yarn_count'][$i])."','".$_REQUEST['yarn_name'][$i]."','".$_REQUEST['yarn_colour'][$i]."','".$_REQUEST['yarn_content'][$i]."','".$_REQUEST['yarn_pcs_weight'][$i]."','".$_REQUEST['consumption'][$i]."')")or die(mysqli_error());
					}
				}
							...
							<div class="card-body">
								<div class="row">
									...
									<div class="form-group row">
										<label for="fname" class=" text-right control-label col-form-label">&nbsp;Indent No</label>
		<div class="col-sm-3">
	...
	<table id="example" class="table table-striped table-bordered" style="width:100%">
		<thead style="background-color: #626F80; color: #fff; font-size: 16px;">
			<tr>
				...
				<th style="width:5%">NOW WANTED (Wgt)</th>
				...
				while ($res_yarns = mysqli_fetch_array($sel_yarns, MYSQLI_ASSOC)) {
					$sel_wgt = mysqli_fetch_array(mysqli_query($zconn, "select * from yarns_po_details
					 where order_no='" . $coldata['order_no'] . "' and styleno='" . $coldata['style_no'] . "'
					  and yarn_name='" . $res_yarns['yarn_name'] . "' and pcs_weight='" . $res_yarns['pcs_weight'] . "'ORDER BY id DESC LIMIT 1 "), MYSQLI_ASSOC);
						$sel_wgt1 = mysqli_fetch_array(mysqli_query($zconn, "select * from costing_entry_master
					 where order_no='" . $coldata['order_no'] . "' and style_no='" . $coldata['style_no'] . "'
					   ORDER BY id DESC LIMIT 1 "), MYSQLI_ASSOC);
							//var_dump($sel_wgt1);


$result_set = mysqli_query(
    $zconn,
    "SELECT * FROM costing_entry_details WHERE costing_no='" . $sel_wgt1['costing_no'] . "' and pcs_weight='" . $res_yarns['pcs_weight'] . "' ORDER BY id DESC"
);

// Check if the query was successful
if ($result_set) {
    // Fetch all rows as an associative array
    $sel_wgt2_array = mysqli_fetch_all($result_set, MYSQLI_ASSOC);

    // Free the result set
    mysqli_free_result($result_set);
} else {
    // Handle the case where the query fails
    echo "Error executing query: " . mysqli_error($zconn);
}

// Loop through the array and echo input fields
foreach ($sel_wgt2_array as $sel_wgt2) {
  //  echo '<input type="text" name="yarn_rate_input[]" value="' . $sel_wgt2['yarn_rate'] . '"><br>';
}

					if ($sel_wgt) {
						$bal = $sel_wgt['balance_qty'];
					} else {
						$bal = $res_yarns['yarn_total']; // If no "PO" balance quantity, set it to an empty string
					}
...
<tr>
	...
	<td style="padding:0px;"><input type="text" name="yarn_content[]" class="form-control" id="yarn_content" value="<?php echo $res_yarns['yarn_content'];?>" ></td>
	<td style="padding:0px;"><input type="text" name="yarn_colour[]" class="form-control" id="yarn_colour" value="<?php echo $res_yarns['yarn_colour'];?>" ></td>
	<td style="padding:0px;"><input type="text" name="yarn_pcs_weight[]" class="form-control" id="yarn_count" value="<?php echo $res_yarns['pcs_weight'];?>" ></td>
		<td style="padding:0px;"><!---?php echo $order['cutting_qty'];?--><input type="text" name="cutting_qty[]" id="cutting_qty" value="<?php echo $order['cutting_qty']; ?>" class="form-control"></td>
		<td style="padding:0px;"><!--?php echo $order['cutting_qty']*$res_yarns['pcs_weight'];?--><?php echo $res_yarns['yarn_total']; ?></td>
	...
	value="<?php echo $bal;?>">
		...
		<td style="padding:0px;">
		<?php	foreach ($sel_wgt2_array as $sel_wgt2) {
   // echo '<input type="text" name="yarn_rate_input[]" value="' . $sel_wgt2['yarn_rate'] . '"><br>';
} ?>
			<input type="text" onkeyup="check_rate('<?php echo $rw;?>');" name="rate[]" id="rate<?php echo $rw;?>" class="form-control" autocomplete="off"  
		value="<?php echo $sel_wgt2['yarn_rate'];?>">
		<input type="hidden"  id="rate1<?php echo $rw;?>" class="form-control" autocomplete="off"  value="<?php echo $sel_wgt2['yarn_rate'];?>"></td>
 	</tr>
	<?php
	$rw++; 
			}
...
<tfoot>
					...
					<div id="supp_msg" class="col-sm-6 text-right" style="color:red; font-size:12px;"></div></div>
						<div class="form-group row">
								...
								<div class="col-sm-6" >
									...
									<option value="">--Select--</option>
									<?php 
				$tocompe = mysqli_query($zconn,"SELECT * FROM  `suppliers` where status='0' and supplier_type_id='1'");
							...
							<div class="form-group row">
									...
									<div class="form-group row">
										...
										<div class="col-sm-6">
											...
											<textarea class="form-control" id="comments" name="comments" autocomplete="off" placeholder="Remarks"></textarea>
									...
									<div class="form-group row">
										...
										<!-- <div class="col-sm-6">
											...
											<?php 
												...
												"SELECT * FROM `supplier_types` where status='0' and supplier_type_id='1'"),MYSQLI_ASSOC);
	...
	$sel_wgt = mysqli_fetch_array(mysqli_query($zconn,"select * from yarns_po_details where order_no='".$coldata['order_no']."' and 
