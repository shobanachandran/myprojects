								...
								<div class="table-responsive" style="overflow-x: auto;">
									<table id="example" class="table table-striped table-bordered" style="width:100%">
										...
										<tbody>
											<?php
											$id = $_GET['id'];
											$sto = mysqli_query($zconn, "select * from yarn_entry_master");
			$rw = 1;
			while ($coldata1 = mysqli_fetch_array($sto, MYSQLI_ASSOC)) {
				$sel_yarns = mysqli_query($zconn, "select * from yarn_entry_details where yarn_id='" . $coldata1['id'] . "'");
											while ($res_yarns = mysqli_fetch_array($sel_yarns, MYSQLI_ASSOC)) {
												echo $yarn_total = $res_yarns['yarn_total'];
											}
			}
											...
											while($coldata = mysqli_fetch_array($secBrnResource,MYSQLI_ASSOC)){
											   ...
											   if ($coldata) {
												  $bal = $coldata['balance_qty'];
											   } else {
												   $bal = $yarn_total; // If no "PO" balance quantity, set it to an empty string
											   }
								...
								<!--td style="padding:0px;"><input readonly  type="text" name="balance[]" id="balance<?php echo $id;?>" 
								class="form-control" value="<?php echo $coldata['balance_qty'];?>">
								<input type="hidden" name="balqty" id="balqty<?php echo $id;?>" value="<?php echo $coldata['balance_qty'];?>"></td-->

								<td style="padding:0px;">
		<input type="hidden" name="balqty" id="balqty<?php echo $sno;?>" 
	value="<?php echo $bal;?>">
		<input type="text" 
		name="balance[]" id="balance<?php echo $sno;?>" 
		class="form-control" value="<?php echo $bal;?>" 
		readonly></td>

							<td>
								
							<!--input type="text" name="now[]" id="now<?php echo $id;?>" value="<?php echo $coldata['weight'];$total_weight+=$coldata['weight']; ?>"  class="form-control now_total" onkeyup="now('<?php echo $id;?>')"-->
							
							
							
							<input type="text" name="now[]" id="now<?php echo $sno;?>" value="<?php echo $coldata['weight'];$total_weight+=$coldata['weight']; ?>"  class="form-control now_total" onkeyup="now('<?php echo $sno;?>')">
						</td>
<!-- 
												<input type="text" name="now" id="now" class="form-control now_wanted" value="<?php echo 10 * $coldata['cutting_qty'];?>"></td> -->

												<td>
													<input type="text" id="rate<?php echo $id;?>" onkeyup="check_rate('<?php echo $id;?>');" name="rate[]" value="<?php echo $coldata['rate'] ?>" class="form-control"><input type="hidden"  id="rate1<?php echo $id;?>" class="form-control" autocomplete="off"  value="<?php echo $coldata['rate'];?>"></td>
