...
<option value="Cutting" <?php  if($_REQUEST['dept']=='Cutting'){?> selected="selected"<?php  }?>>Cutting</option>
<option value="Power" <?php  if($_REQUEST['dept']=='Power') { $Nameworkpen="power_workpen"; ?> selected="selected" <?php }?>>Power Table</option>
<option value="Singer" <?php  if($_REQUEST['dept']=='Singer') {$Nameworkpen="singer_workpen"; ?> selected="selected" <?php } ?>>Singer</option>
<option value="Ironing & packing"<?php  if($_REQUEST['dept']=='Ironing & packing' || $_REQUEST['dept']=='ironing' ) { $Nameworkpen="iron_workpen";?> selected="selected" <?php } ?> >Ironing & packing</option>
...
<option value="washing" <?php if($_REQUEST['dept']=='washing') {$Nameworkpen="washing_workpen";?> selected="selected" <?php } ?>>Washing</option>
                      ...
                      <tr>
                        <td colspan="2" align="center">
                          ...
                          ?>
							...
							$persondetail=mysqli_fetch_object(mysqli_query("SELECT * FROM  `persondetail` where `id`='".$res->contid."' "));
                            if($asd=='0'){
							?>
                              	<li class="active"><a href="#<?php $res->contid?>"><?php $res->contid?><?php  echo("(".$persondetail->name.")");?></a></li>
                             <?php  
							}else{
							?>
								<li><a href="#<?php $res->contid?>"><?php $res->contid?><?php  echo("(".$persondetail->name.")");?></a></li>
							<?php  
							 }
							 $asd=1;
                             } 
                            ...
                            <?php  
                            $sql=mysqli_query("SELECT DISTINCT(contid) as contid  FROM `dummy_contractor` WHERE `dept` LIKE '".$_REQUEST['dept']."' ORDER BY `dummy_contractor`.`contid` ASC");
							$ads=0;
                            while($res=mysqli_fetch_object($sql))
                            {
                                $persondetail=mysqli_fetch_object(mysqli_query("SELECT * FROM  `persondetail` where `id`='".$res->contid."' "));
								...
								<div class="tab-pane active" id="<?php $res->contid?>" >
                            	
                  		  		  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                            	<tr>
                            		<td align="center" valign="top"> <table class="table table-striped table-bordered" width="95%" border="1" align="center" cellpadding="0" cellspacing="0" class="curvedEdges1">
                              <tr>
                            ...
                            <?php 		
                            $sql_result=mysqli_query("SELECT * FROM  `dummy_contractor` WHERE  `contid` =  '".$res->contid."'ORDER BY  `dummy_contractor`.`date` DESC ");
                            while($result_sql=mysqli_fetch_object($sql_result))
                            {
                                ?>
                            
                              <tr>
                                <td align="center" valign="middle"><?php date("d-m-Y",strtotime($result_sql->date))?></td>
                    ...
                    <a class="btn btn-success" href="#" onclick="window.open('iframe_accounts.php?ids=<?php $result_sql->id?>&status=<?php $status?>&pay_date=<?php $result_sql->pay_date?>','Print','fullscreen=yes','_blank')"><i class="icon-list-alt icon-white"></i> View & Print</a>
                                     
            <!--<a href="#" onclick="window.open('iframe_accounts.php?ids=<?php $result_sql->id?>&print=yes&status=<?php $status?>','Print','_blank');" class="btn btn-warning"><i class="icon-print icon-white"></i> Print</a>--></td>
                                </tr>
                                <?php 
                            }
                            ?>   
                                ...
                                <div class="tab-pane" id="<?php $res->contid?>">
                            ...
                            while($result_sql=mysqli_fetch_object($sql_result))
                            {
                              ...
                              <tr>
                                ...
                                <!--<a href="#" onclick="window.open('iframe_accounts.php?ids=<?php $result_sql->id?>&print=yes&status=<?php $status?>','Print','_blank');" class="btn btn-warning"><i class="icon-print icon-white"></i> Print</a>-->
                                </td>
                              </tr>
                                <?php 
                            }
