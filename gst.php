...
<head>
    ...
    <title><?php echo SITE_TITLE;?> - Banking</title>
    <!-- Custom CSS -->
	<!--  datatables CSS -->
							...
							<div class="card-body">
								<div class="table-responsive">
									<table id="example" class="table table-striped table-bordered">
										...
										<tbody>
											<?php
						$sectUsrQry = "SELECT * FROM gst  ORDER BY gst_id desc";
						$secUsrResource = mysqli_query($zconn,$sectUsrQry);
						$ut=1;
						while($userdata = mysqli_fetch_array($secUsrResource,MYSQLI_ASSOC)){
					?>
					<tr id="delete-<?php echo $userdata['gst_id'];?>">
						<td><?php echo $ut; ?></td>
						<td><?php echo $userdata['gst_name']; ?></td>
						<td> <?php echo $userdata['gst_value']; ?></td>
						<td><?php echo $userdata['status']; ?></td>
						<td><a href="gst_add.php?typeid=<?php echo $userdata['gst_id']; ?>"><i class="fa fa-fw fa-edit"></i></a>  | <a onclick="DeleteUsrId('<?php echo $userdata['gst_id']; ?>')"><i class="fa fa-fw fa-times"></i></a> 
					   <?php //} ?>
						</td>
					</tr>
					<?php
	  ...
	  if(UsrStatus){
		$.ajax({
			...
			success: function( data ) {
				...
				if($.trim(data)=="exist"){
					alert("GST Exist in the User(s) Data!!");
					$('.loader').hide();
				}
