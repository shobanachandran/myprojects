 ...
 if (!empty($_FILES['order_image']['tmp_name'])) {
		...
		for($ts=0;$ts<$order_qty_count;$ts++){
			for($tb=0;$tb<$order1_sizes;$tb++){
				...
				$order_child = mysqli_query($zconn,"insert into order_quantity_details(order_id,order_no,po_no,row_id,color,size_id,qty_val) values('".$last_insert_id."','".$_REQUEST['order_no']."','".$_REQUEST['po_no']."','".$ts."','".$order_colour[$ts]."','".$szeid."','".$_REQUEST['order_sizes'.$szeid][$ts]."')");
			}
		}
		...
		for($ts=0;$ts<$cutting_qty_count;$ts++){
			for($tb=0;$tb<$order2_sizes;$tb++){
				...
				row_id,color,size_id,qty_val) values('".$last_insert_id."','".$_REQUEST['order_no']."','".$_REQUEST['po_no']."',
...
<body>
                ...
                <div class="input-group-append">
                    <button class="btn btn-outline-secondary add-color" type="button">+</button>
									...
									<div class="form-group row">
										...
										<div class="col-sm-6">
											<input type="text" class="form-control" id="order_no" name="order_no" autocomplete="off" required placeholder="Indent Number" value="<?php echo $_REQUEST['order_no'];?>">
										</div>
									</div>
									<div class="form-group row">
									 ...
									 <button type="button" class="btn btn-info add-new-row"><i class="fa fa-plus"></i></button> 
										...
										<tbody>
											...
											<?php 
												...
												<select name="order_colour[]" style="width:150px;" class="select2 form-control custom-select chosen-select">
    <?php
												...
												$(".chosen-select").chosen({
											  	no_results_text: "Oops, nothing found!"
													...
													for($ij=0;$ij<count($sizeids);$ij++){	?>
														...
														<input type="text" name="order_sizes<?php echo $sizeids[$ij];?>[]" style="width:150px;" class="orderqty" onkeyup="cal_orderqty(this);cal_orderqty2(this);cal_total_cutting_qty();" onchange="">
    ...
    row.querySelector('.total-sum4').textContent = sum;
		...
		<?php $sel_color = mysqli_query($zconn,"select * from color_groups where color_group='".$_REQUEST['color_group']."'");
			...
			<tr><td><!--input type="text"  name="cutting_colour[]"  value="<?php echo $sql_color1['colour_name'];?>"-->
				...
				<select  name="cutting_colour[]"  style="width:150px;" class="select2 form-control custom-select chosen-select">
    <?php
    // Assuming you have a database connection established
    ...
    ?>
				...
				for($ij=0;$ij<count($sizeids);$ij++){	?>
					...
					<input type="text"  name="cutting_sizes<?php echo $sizeids[$ij];?>[]" style="width:150px;" class="cuttingqty row-total" onkeyup="cal_orderqty1(this);cal_orderqty3(this);cal_total_cutting_qty()" >
					</td>
					
				<?php } ?>
				<td class="total-sum row-total"></td>
...
while ($row_color = mysqli_fetch_array($sql_color, MYSQLI_ASSOC)) {
    $color_list1 .= '<option value="' . $row_color['colour_name'] . '">' . $row_color['colour_name'] . '</option>';
}
...
$(".add-new-row").on('click', function() {
   ...
   //var orderRow = '<td><select name="order_colour[]" class="select2 form-control custom-select chosen-select">';
<?php
$sql = "SELECT * FROM color_master WHERE status = '0'";
$result = mysqli_query($zconn, $sql);

// Loop through the database results in PHP and create JavaScript code to populate the select dropdown
while ($color = mysqli_fetch_assoc($result)) {
   // echo "orderRow += '<option value=\"" . $color['colour_name'] . "\">" . $color['colour_name'] . "</option>';\n";
}
?>
//orderRow += '</select></td>';

orderRow += '<td><select name="order_colour[]" style="width:150px;"class="select2 form-control custom-select chosen-select"><option value="">--Select--</option><?php echo $color_list1; ?></select></td>';

    <?php
    for ($ij = 0; $ij < count($sizeids); $ij++) { ?>
        orderRow += '<td><input type="text" name="order_sizes<?php echo $sizeids[$ij];?>[]" style="width:150px;"  class="orderqty" onkeyup="cal_orderqty(this);cal_total_cutting_qty();cal_orderqty4(this)"></td>';
    <?php } ?>
    orderRow += '<td class="order-total-sum"></td>';
  ...
  // var cuttingRow = '<td><select name="cutting_colour[]" class="select2 form-control custom-select">';
<?php
$sql = "SELECT * FROM color_master WHERE status = '0'";
$result = mysqli_query($zconn, $sql);
...
// Loop through the database results in PHP and create JavaScript code to populate the select dropdown
while ($color = mysqli_fetch_assoc($result)) {
   // echo "cuttingRow += '<option value=\"" . $color['colour_name'] . "\">" . $color['colour_name'] . "</option>';\n";
}
?>

//cuttingRow += '</select></td>';

cuttingRow += '<td><select name="cutting_colour[]" style="width:150px;" class="select2 form-control custom-select chosen-select"><option value="">--Select--</option><?php echo $color_list1; ?></select></td>';


    <?php
    for ($ij = 0; $ij < count($sizeids); $ij++) { ?>
        cuttingRow += '<td><input type="text" name="cutting_sizes<?php echo $sizeids[$ij];?>[]" style="width:150px;" class="cuttingqty" onkeyup="cal_cuttingqty(this);cal_orderqty5(this);cal_total_cutting_qty()"></td>';
    <?php } ?>
    cuttingRow += '<td class="total-sum5 row-total">';
...
$("table").on('click', '.remove-row', function() {
    ...
    var totalOverallCuttingQty = 0;

    // Calculate row-wise totals
    $('tr').each(function() {
        var totalCuttingQty = 0;
        $(this).find('.cuttingqty').each(function() {
            var val = $(this).val();
            if (!isNaN(val) && val !== '') {
                totalCuttingQty += parseFloat(val);
            }
        });
        $(this).find('.row-total').text(totalCuttingQty); // Display row total
        totalOverallCuttingQty += totalCuttingQty; // Calculate overall total
    });

    // Display overall total in the input field with class .total-cutting-qty
    $('.total-cutting-qty').val(totalOverallCuttingQty);
}

// Call the function initially to calculate totals
cal_total_cutting_qty();
