...
<body>
    ...
    <div class="col-sm-6">
        <textarea autocomplete="off" class="form-control" id="desc" name="description"></textarea>
									...
									<div class="form-group row">
										...
										<input type="text" autocomplete="off" required class="form-control" id="order_qty" name="order_qty" value="<?php echo $_REQUEST['order_qty'];?>" placeholder="Order Quantity">
													<input type="text" autocomplete="off" required class="form-control" id="cutting_qty" name="cutting_qty" value="<?php echo $_REQUEST['cutting_qty'];?>" placeholder="Cutting Quantity">
										</div>
									</div>
									<div class="form-group row">
									 ...
									 <button type="button" class="btn btn-info add-new-row"><i class="fa fa-plus"></i></button> 
										...
										<tbody>
											...
											<?php 
												...
												?>
												<tr><td>
												<input type="text"  name="order_colour[]"  value="<?php echo $sql_color1['colour_name'];?>"></td>
												
													<?php
													for($ij=0;$ij<count($sizeids);$ij++){	?>
														<td>
					
														<input type="text" name="order_sizes<?php echo $sizeids[$ij];?>[]" style="width:100px;" class="orderqty" onkeyup="cal_orderqty(this);">
		...
		<?php $sel_color = mysqli_query($zconn,"select * from color_groups where color_group='".$_REQUEST['color_group']."'");
			...
			$sql_color1 = mysqli_fetch_array(mysqli_query($zconn,"select * from color_master where id='".$color_arr[$co]."'"),MYSQLI_ASSOC);
			?>
			<tr><td><input type="text"  name="cutting_colour[]"  value="<?php echo $sql_color1['colour_name'];?>"></td>
				<?php
				for($ij=0;$ij<count($sizeids);$ij++){	?>
					<td>
			
					<input type="text"  name="cutting_sizes<?php echo $sizeids[$ij];?>[]" style="width:100px;" class="cuttingqty" onkeyup="cal_orderqty(this);">
					</td>
...
$(".add-new-row").on('click', function() {
    ...
    // Code for adding rows in the order quantity table
    orderRow += '<td><input type="text"  name="order_colour[]" value="<?php echo $sql_color1['colour_name'];?>"></td>';
    <?php
    for ($ij = 0; $ij < count($sizeids); $ij++) { ?>
        orderRow += '<td><input type="text" name="order_sizes<?php echo $sizeids[$ij];?>[]" style="width:100px;" class="orderqty" onkeyup="cal_orderqty(this);"></td>';
    <?php } ?>
    ...
    <?php
    for ($ij = 0; $ij < count($sizeids); $ij++) { ?>
        cuttingRow += '<td><input type="text" name="cutting_sizes<?php echo $sizeids[$ij];?>[]" style="width:100px;" class="cuttingqty" onkeyup="cal_cuttingqty(this);"></td>';
    <?php } ?>
    cuttingRow += '<td class="cutting-total-sum">';
...
$("table.order-table").on('input', '.orderqty', function() {
    ...
    row.find('.order-total-sum').text('Order Total: ' + totalOrder);
});
...
$("table.cutting-table").on('input', '.cuttingqty', function() {
    ...
    row.find('.cutting-total-sum').text('Cutting Total: ' + totalCutting);
});
...
<?php
		...
		$sel_color = mysqli_query($zconn,"select * from color_groups where color_group='".$_REQUEST['color_group']."'");
			...
			$col_name = $sql_color1['colour_name'];
			$color_list .='<option value='.$col_name.'>'.$sql_color1['colour_name'].'</option>';
		} 
		$size_list='';
		$size_sql = mysqli_query($zconn,"select * from size_groups where size_group_name='".$_REQUEST['size_group']."'");
		$res_size = mysqli_fetch_array($size_sql,MYSQLI_ASSOC);	
		$sizeids = explode(",",$res_size['size_ids']);
		for($ij=0;$ij<count($sizeids);$ij++){
			$size_list .= "<td><input type='text' name='order_sizes".$sizeids[$ij]."[]' style='width:100px;' class='orderqty' onkeyup='cal_orderqty();' ></td>";
		}
		...
		$sizeids = explode(",",$res_size['size_ids']);
		for($ij=0;$ij<count($sizeids);$ij++){
			$size_list1 .= "<td><input type='text' name='cutting_sizes".$sizeids[$ij]."[]' style='width:100px;'  class='cuttingqty' onkeyup='cal_cuttingqty();'></td>";
		}
		*/
	?>
