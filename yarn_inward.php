...
for($i=0;$i<$count;$i++)
	...
	if ($_REQUEST['wgt'][$i]>0) {
			$wgt=$_REQUEST['wgt'][$i];
			$fabrics0=mysqli_fetch_object(mysqli_query($zconn,"select sum(weight) as weight from  yarns_po_details where `po_id`='".$_REQUEST['po_no']."' and id='".$_REQUEST['id'][$i]."' and order_no= '".$_REQUEST['order_no'][$i]."'"));
			 $fwgt=$fabrics0->weight;
			$fabrics=mysqli_fetch_object(mysqli_query($zconn,"select sum(wgt) as wgt from  yarn_inward where `po_no`='".$_REQUEST['po_no']."' and old_id='".$_REQUEST['id'][$i]."' and order_no= '".$_REQUEST['order_no'][$i]."'"));
			$new=$_REQUEST['wgt'][$i];
			$fwgt2=$fabrics->wgt+$new;
			$tcom=$fwgt-$fwgt2;

			if ($tcom==0) {
				mysqli_query($zconn,"UPDATE  `yarns_po_details` SET  `status` =  'complete' where `po_id`='".$_REQUEST['po_no']."' and id='".$_REQUEST['id'][$i]."' and order_no= '".$_REQUEST['order_no'][$i]."'")or die(mysqli_error());
			}
$sql = mysqli_query($zconn, "INSERT INTO `yarn_inward` (`po_no`, `style_no`, `order_no`, `wgt`, `roll`, `date`, `old_id`, `inward_wgt`) VALUES ('".$_REQUEST['po_no']."', '".$_REQUEST['style_no'][$i]."', '".$_REQUEST['order_no'][$i]."', '".$_REQUEST['wgt'][$i]."', '".$_REQUEST['roll'][$i]."', NOW(), '".$_REQUEST['id'][$i]."', '".$_REQUEST['wgt'][$i]."')") or die(mysqli_error());
...
<head>
    ...
    <title><?php echo SITE_TITLE;?> - YARN PO INWARD</title>
    <!-- Custom CSS -->
	<!--  datatables CSS -->
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
             ...
             <div class="page-breadcrumb">
                <div class="row">
                    <div class="col-12 d-flex no-block align-items-center">
                        <h4 class="page-title">YARN PO INWARD</h4>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-md-12">
                        <div class="card">
							...
							<div class="row">
								...
								<div class="col-sm-6">
								<select class="select2 form-control custom-select chosen-select" name="po_no" id="po_no" onchange="$('#yarn_inward').submit();"> <option>Select</option>
								<?php $sel_po = mysqli_query($zconn,"select distinct po_id,styleno  from  yarns_po_details where status!='complete'");
			...
			<div class="col-sm-2"><strong> Party Name</strong></div>
				<div class="col-sm-3">
			<?php
				$sel_buyer = mysqli_fetch_object(mysqli_query($zconn,"select * from yarns_po_master where po_no='".$_REQUEST['po_no']."'"));
		...
		<hr>
			<div class="table-responsive">
				<div class="col-12 d-flex no-block align-items-center">
					<h5 class="page-title" style="margin-left: 390px;">YARN DETAILS</h5>
				</div>
				<div class="row">
					...
					<div class="col-sm-3">
						 <select class="select2 form-control custom-select" name="order" id="order" onchange="this.form.submit();">
								...
								<?php $sel_buyer = mysqli_query($zconn,"select * from yarns_po_details where po_id='".$_REQUEST['po_no']."' group by order_no");
					...
					<div class="col-sm-3">
						 <select class="select2 form-control custom-select" name="style" id="style" onchange="this.form.submit();">
								<option value="0">Select</option>
								<?php $sel_buyer = mysqli_query($zconn,"select * from yarns_po_details where po_id='".$_REQUEST['po_no']."' and order_no='".$_REQUEST['order']."' group by order_no,styleno");
								while($res_buyer = mysqli_fetch_array($sel_buyer,MYSQLI_ASSOC)){ ?>
				...
				<table id="example" class="table table-striped table-bordered" style="width:100%">
					<thead style="background-color: #626F80; color: #fff; font-size: 16px;">
						<tr>
							...
							<th style="width: 9%" data-toggle="tooltip" title="Total Weight">TOT. WEIGHT</th>
							<th style="width: 7%" data-toggle="tooltip" title="Already Received">ALR.REC.</th>
							<th style="width: 3%" data-toggle="tooltip" title="Balance Quantity">BAL.</th>
							<th style="width: 6%" data-toggle="tooltip" title="Received Bags">REC.BAGS</th>
							<th style="width: 8%" data-toggle="tooltip" title="Received Weight">REC.WEIGHT</th>
						</tr>
					</thead>
					<tbody>
						<?php
						$query = "SELECT * FROM yarns_po_details where po_id='".$_REQUEST['po_no']."' and status!='complete'";
						...
						while($coldata = mysqli_fetch_array($secBrnResource,MYSQLI_ASSOC)){
							$id=$coldata['id'];
							$fetch=mysqli_fetch_object(mysqli_query($zconn,"select sum(wgt) as wgt,sum(roll) as roll from  yarn_inward where po_no='".$_REQUEST['po_no']."' and old_id=$id"));
