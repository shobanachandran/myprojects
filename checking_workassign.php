 ...
 <!-- Add the appropriate action attribute for your form -->
        ...
        if (isset($_POST['order'])) { // Check if an "order_no" is set in the form
            ...
            $query = "SELECT * FROM singer_workdone WHERE `order_no` = '$orderNo'";
            ...
            if ($result) {
                ...
                $sectBrnQry = "SELECT DISTINCT size_id FROM singer_workdone WHERE order_no='" . $_REQUEST['order'] . "'";
                ...
                $sectBrnQry = "SELECT DISTINCT color FROM singer_workdone WHERE order_no='" . $_REQUEST['order'] . "'";
                $secBrnResource = mysqli_query($zconn, $sectBrnQry);
                ...
                $sectBrnQry = "SELECT * FROM singer_workdone WHERE order_no='" . $_REQUEST['order'] . "'";
                $secBrnResource = mysqli_query($zconn, $sectBrnQry);
...
<div class="table-responsive">
    ...
    <table class="table table-striped table-bordered text-center">
        ...
        <tbody>
            ...
            $sectBrnQry = "SELECT order_no, color, size_id, qty_val FROM singer_workdone WHERE order_no='" . $_REQUEST['order'] . "'";
            $secBrnResource = mysqli_query($zconn, $sectBrnQry);
            while ($coldata = mysqli_fetch_array($secBrnResource, MYSQLI_ASSOC)) {
            ...
            $sectBrnQry = "SELECT order_no, color, size_id, SUM(qty_val) AS total_qty FROM checking_workassign WHERE order_no='" . $_REQUEST['order'] . "' GROUP BY order_no, color, size_id";
            $secBrnResource = mysqli_query($zconn, $sectBrnQry);
            while ($coldata = mysqli_fetch_array($secBrnResource, MYSQLI_ASSOC)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save'])) {
    ...
    if (!empty($insertData)) {
        foreach ($insertData as $data) {
            ...
            // Insert data into the 'checking_workassign' table
            $insertQuery = "INSERT INTO checking_workassign (style_no, order_no, dc_no, to_contractor, `from`, dc_date, size_id, color, qty_val, `session_entry`) VALUES ('$style_no', '$order_no', '$dc_no', '$to_contractor', '$from', '$dc_date', '$size_id', '$color', $qty_val, '$uname')";
            ...
            if (!$result) {
                ...
                die("An error occurred while inserting data into checking_workassign.");
            }
