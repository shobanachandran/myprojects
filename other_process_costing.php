...
for ($i = 0; $i < $fabric_name_count; $i++) {
    ...
    //$count = mysqli_num_rows($sel_check);
	 ...
	 $insert_fab_costing = mysqli_query($zconn, "INSERT INTO other_costing (costing_no, process_name, subprocess_name, weight, rate, rate_pcs, total, created_by, created_date, fabric_name, process_loss)
            VALUES ('$current_costing_no', '$current_process_name', '...', '$current_wgt', '$current_rate', '$current_rate_pcs', '$current_total', '$_SESSION[userid]', NOW(), '$current_fabric_name', '...')");
								...
								<div class="card-body" style="width:100%">
									<div class="card-body" style="width:100%">
										...
										<option>Select</option>
												...
												<?php if($_REQUEST['id']==''){
						$sel_costing = mysqli_query($zconn,"select * from costing_entry_master where id NOT IN(select costing_no from other_costing_list)AND cost_type = 'yarn' ");
												} else {
							$sel_costing = mysqli_query($zconn,"select * from costing_entry_master where id IN(select costing_no from other_costing_list)AND cost_type = 'yarn' ");						
												}
	...
	<option value="">Select</option>
		<?php echo $sublist; ?>
					...
					<?php if(count($fabrics) > 1) { ?>
							...
							<input type="text" style="display:none"  class="form-control total" data-no="<?php echo $f; ?>" id="total_hidden<?php echo $f; ?>" value="<?php  echo $res_costing1['total'];$total_rate; $tot+=$total_rate;?>" name="total[]" readonly placeholder="Total">
							<?php } ?>
...
<td style="width: 15%" >&nbsp;</td><td style="width: 15%">&nbsp;</td><td style="width: 15%">&nbsp;</td><td style="width: 11%">&nbsp;</td><td style="width: 11%">&nbsp;</td>
												...
												<td style="width: 11%"><input type="text" name="total_amount" class="form-control" id="total_rate" value="<?php echo $tot;?>" readonly placeholder="" style="border: 1px solid #000;"></td>
